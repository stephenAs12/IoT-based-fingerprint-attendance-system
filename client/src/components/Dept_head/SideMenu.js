...
import {SidebarData4} from './SidebarData';
import {SidebarData5} from './SidebarData';
...
let rows_for_registrar = [{college: "", email: "", first_name: "", id: "", last_name: "", middle_name: "", password: "", phone_number: "", role: "", sex: ""}];
...
export default function SideMenu() {
  ...
  React.useEffect(() => {
    if(localStorage.getItem("identify-logged-user")) {
     ...
     console.log("user at local storage ",user);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    Axios.post("http://localhost:3001/view_head_info_in_head_for_auth", {
      email: temp_email,
    }).then((response) => {
      ...
      if(response.data.length>0){
        ...
        setLoggedUserData(rows_for_registrar);
        console.log("loggedUserData ",loggedUserData);
      }
...
React.useEffect(() => {
  localStorage.setItem("identify-head-college", JSON.stringify(collegeOfRegistrar));
});
...
// import MuiAccordionDetails from '@material-ui/core/AccordionDetails';
// import {userEmail} from '../login';
// import {headCollege} from '../login';
// import {headDepartment} from '../login';
// import CustomizedDialogs from '../dialog';
// import "./head.css";
...
//     width: theme.spacing(7) + 1,
//     [theme.breakpoints.up('sm')]: {
//       width: theme.spacing(9) + 1,             //  sidebar size
//     },
//   },
//   toolbar: {
...
//       const head_college = JSON.parse(localStorage.getItem("identify-head-college"));
//      console.log("head_college "+head_college);
//     }
//   }, []);

//   React.useEffect(() => {
//     if(localStorage.getItem("identify-head-department")) {
//       const head_department = JSON.parse(localStorage.getItem("identify-head-department"));
//      console.log("head_department "+head_department);
//     }
//   }, []);

//   React.useEffect(() => {
//     localStorage.setItem("identify-head-college", JSON.stringify(headCollege));
//   });

//   React.useEffect(() => {
//     localStorage.setItem("identify-head-department", JSON.stringify(headDepartment));
//   });
